...
import { generateAiLevel } from './services/geminiService.js';
import { Trophy, Skull, Play, Cat, Sparkles, MousePointer2, MoveUp, FastForward, Rewind } from 'lucide-react';

const App = () => {
  const [status, setStatus] = useState('MENU');
  const [levelNumber, setLevelNumber] = useState(1);
  const [currentLevel, setCurrentLevel] = useState(DEFAULT_LEVELS[0]);
  ...
  const playerTarget = useRef({ x: 0, y: 0 });
  const enemyTargets = useRef(new Map());
  const keysPressed = useRef(new Set());
  const spaceWasDown = useRef(false);
  const pWasDown = useRef(false);
  const gameLoopRef = useRef(null);

  const playOneShot = useCallback((url, volume = 0.5) => {
    const audio = new Audio(url);
    audio.volume = volume;
    audio.play().catch(() => {});
  }, []);

  const isStandingOnSolid = useCallback((x, y, currentGrid, currentDigs) => {
  ...
  const isInsideLadder = useCallback((x, y, currentGrid) => {
    const ix = Math.round(x);
  ...
  const canMoveTo = useCallback((x, y, currentGrid, currentDigs) => {
    const ix = Math.round(x);
    const iy = Math.round(y);
    if (ix < 0 || ix >= GRID_WIDTH || iy < 0 || iy >= GRID_HEIGHT) return false;
  ...
  const initLevel = useCallback((level, num) => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-950 text-white p-4">
      <style>{`
        @keyframes psy-brick { 0% { background-color: #f00; filter: hue-rotate(0deg); } 50% { background-color: #0f0; filter: hue-rotate(180deg); } 100% { background-color: #f00; filter: hue-rotate(360deg); } }
      ...
      <div className="w-full max-w-4xl flex justify-between items-start mb-4 px-6 bg-zinc-900/90 py-3 rounded-2xl border border-zinc-800 shadow-2xl backdrop-blur-xl">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex gap-2">
            <button onClick={handlePreviousLevel} className="p-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-colors"><Rewind size={18}/></button>
            <button onClick={handleNextLevel} className="p-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-colors"><FastForward size={18}/></button>
          </div>
        </div>
      </div>
      ...
      <div className="relative w-full max-w-4xl aspect-[20/15] bg-black rounded-2xl overflow-hidden border-8 border-zinc-900 shadow-2xl ring-1 ring-white/10">
        ...
        {status === 'MENU' && (
          <div className="absolute inset-0 bg-black/95 flex flex-col items-center justify-center z-50 p-8 text-center">
            ...
            <p className="text-zinc-400 mb-8 max-w-md font-medium text-lg">100 unique levels of feline mayhem. Snag the fish. Dodge the dogs.</p>
            <button onClick={() => initLevel(DEFAULT_LEVELS[0], 1)} className="bg-orange-600 hover:bg-orange-500 px-12 py-5 rounded-2xl font-black text-2xl flex items-center gap-3 transition-all transform hover:scale-105 shadow-xl shadow-orange-900/40"><Play fill="white"/> START HUNT</button>
          </div>
        )}
        ...
        {status === 'GAMEOVER' && (
          <div className="absolute inset-0 bg-rose-950/90 flex flex-col items-center justify-center z-50 p-8 text-center backdrop-blur-sm">
            ...
            <h2 className="text-4xl font-black pixel-font text-white mb-8">CAUGHT!</h2>
            <div className="flex gap-4">
              <button onClick={() => initLevel(currentLevel, levelNumber)} className="bg-white text-rose-950 px-10 py-4 rounded-2xl font-black text-xl hover:bg-zinc-200 transition-all">RETRY</button>
        ...
        {status === 'VICTORY' && (
          <div className="absolute inset-0 bg-emerald-950/90 flex flex-col items-center justify-center z-50 p-8 text-center backdrop-blur-md">
            <Trophy size={90} className="text-yellow-400 mb-6 floating" />
            <h2 className="text-5xl font-black pixel-font text-white mb-10">MEOW-VELOUS!</h2>
            <button onClick={handleNextLevel} className="bg-white text-emerald-950 px-14 py-5 rounded-2xl font-black text-2xl hover:bg-zinc-200 transition-all">NEXT LEVEL</button>
          </div>
        )}
      ...
      <footer className="mt-8 flex flex-wrap justify-center gap-6 text-[11px] font-black uppercase text-zinc-500 tracking-widest">
        <div className="flex items-center gap-2 bg-zinc-900/80 px-5 py-2.5 rounded-full border border-zinc-800"><MousePointer2 size={12}/> ARROWS: MOVE</div>
        <div className="flex items-center gap-2 bg-zinc-900/80 px-5 py-2.5 rounded-full border border-zinc-800 text-orange-400"><Sparkles size={12}/> SPACE: DOUBLE JUMP</div>
        <div className="flex items-center gap-2 bg-yellow-950/20 px-5 py-2.5 rounded-full border border-yellow-900/50 text-yellow-400"><MoveUp size={12}/> P: PEE (SLOW DOGS)</div>
        <div className="flex items-center gap-2 bg-zinc-900/80 px-5 py-2.5 rounded-full border border-zinc-800">Z/X/S: DIG & CLAW</div>
      </footer>
    </div>
  );
};
